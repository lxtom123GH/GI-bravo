...
- **Security rules** (`firestore.rules`): per-user data under `apps/{appId}/users/{uid}/**` (owner
  ...
  now binds each row's doc-id to the caller's own token email **and requires a verified email**
  (`SEC-2a` + hardening) — you can only claim the index entry for *your own verified* email, so nobody
  can hijack someone else's or squat an unclaimed one; covered by rules tests. Sign-up sends a
  verification link (`js/sync/auth.js`); Google is always verified, Email-Password users get their
  index row on the first sign-in after verifying (personal sync works regardless).
